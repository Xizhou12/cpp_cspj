...
/*
    ...
    我们有两种函数
    built-in functions 内置函数 例如 strlen printf scanf 等是c++自带的函数， 只能调用不能修改
    自定义函数 - 我们自己定义的函数

    注意：
        c++函数内不能在定义函数 所以函数和函数之间都是分开的，
        要注意函数的书写格式 小括号 大括号看清楚分别在哪里
        main 函数是程序的入口，程序从main函数开始执行 所以如果没有在main函数调用的函数是不会执行的
*/
...
int main() { // 主函数
    ...
    */
    ...
    /*
        函数过载这一特点让add4可以接受不同类型，个数的参数
    */
    cout << add4(1,2,3) << endl; // 6
    cout << add4(1.1, 2.2) << endl; // 3.3
    
    /*
        使用模板以后 add4 可以接受任何类型的参数
    */
    cout << add5(1, 2) << endl;
    cout << add5(1024.7, 512.3) << endl;
    return 0;
